...
{
  pres = all_acc %>% group_by(class) %>%
    rstatix::wilcox_test(accuracy ~ normalized) %>%
    rstatix::add_xy_position() %>% rstatix::add_significance()
  pres$y.position = c(1.3, 1.2, 1.1)
  pres$xmin = c(.75, 1, 1.25)
  pres$xmax = c(1.75, 2, 2.25)
  
  ggplot(all_acc, aes(x=normalized, fill=class, y=accuracy)) +
    geom_boxplot(lwd=0.25, outlier.size = .5) + ggtitle(tstr) +
    theme(plot.title = element_text(hjust = 0.5)) +
    ylim(0.3, 1.4) +
    stat_pvalue_manual(pres, label = 'p')

  ggsave(filename = paste0(outname, '_accuracy_5.pdf'), width = 4, height = 5)
}
...
for (i in input_set:input_set)
{
  ...
  load(paste0(suboutdir,'/kallisto_',fname,'_','none','_','none','_pca_res.RData'))
  sub_raw_data = t(pca.res$x[, 1:10])
  ...
  all_acc_raw_svm_rad = run_one_sample_raw(predict_lbl_svm_rad,
			   sub_raw_proj, sub_raw_tiss, sub_raw_gns)
  ## Logistic Regression
  all_acc_raw_lr = run_one_sample_raw(predict_lbl_lr,
			   sub_raw_proj, sub_raw_tiss, sub_raw_gns)
  ...
  for (c_method in c_method_names)
  {
	...
	next
      ...
      load(paste0(suboutdir,'/kallisto_',fname,'_',n_method,'_',c_method,'_pca_res.RData'))
      sub_nrm_data = t(pca.res$x[, 1:10])
      ...
      ## Logistic Regression
      all_acc = run_one_sample_nrm(predict_lbl_lr,
			       sub_nrm_proj, sub_nrm_tiss, sub_nrm_gns)
      all_acc = rbind(all_acc_raw_lr, all_acc)
      outname = paste0(suboutdir,'/lr_100r_',fname,'_',n_method,'_',c_method)
      make_one_box(all_acc, outname, paste0('LR, Normalize:',n_method,'+Batch:',c_method,', ',tissues_tstr))
    }
  }
}
